...
void * func1(void * s)
{
	...
	if(counter==n){
		sem_post(&barrier);
        printf("%d was the last one ; " , (char* )s);
        }
	sem_wait(&barrier);
	sem_post(&barrier);          
...
// void * func2(void *s)
// {
// 	printf("Entered func2 with attribute:%s Now it's time to waitfor others.\n",(char *)s);
...
//         printf("func2 with attribute:%s Ended!\n",(char *)s);
...
int main()
{
    ...
    pthread_create(&t0,0,func1,"jafar");
	pthread_create(&t1,0,func1,"asqar");
    pthread_create(&t2,0,func1,"akbar");
    pthread_create(&t3,0,func1,"qolli");
